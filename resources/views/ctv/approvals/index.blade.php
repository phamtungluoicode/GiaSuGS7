...
@section('title', 'Phê duyệt gia sư - CTV')
                ...
                @forelse($pendingTeachers as $index => $teacher)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            <a href="{{ route('ctv.approvals.show', $teacher->id) }}" class="inline-flex items-center px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded hover:bg-blue-700 transition">
                                Xem
                            </a>
                            <form action="{{ route('ctv.approvals.approve', $teacher->id) }}" method="POST" class="inline">
                                @csrf
                            ...
                            <form action="{{ route('ctv.approvals.reject', $teacher->id) }}" method="POST" class="inline" data-confirm="Bạn có chắc chắn muốn từ chối?" onsubmit="return confirm('Bạn có chắc chắn muốn từ chối gia sư này?')">
                                @csrf
