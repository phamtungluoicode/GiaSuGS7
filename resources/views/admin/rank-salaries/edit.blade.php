...
@section('title', 'Sửa mức lương - Admin')
@section('header', 'Cập nhật mức lương')

@section('content')
<div class="max-w-lg mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin.rank-salaries.update', $rankSalary->id) }}">
            @csrf
            @method('PUT')

            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Mức lương</label>
                <input type="text" name="name" id="name" value="{{ old('name', $rankSalary->name) }}"
